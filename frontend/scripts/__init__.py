...
PROJECT_ROOT = ROOT_DIR.parent  # プロジェクトルート
...
def build():
    ...
    # amplify_outputs.jsonからAPI URLを読み取り、config.pyを生成
    amplify_outputs_path = PROJECT_ROOT / "amplify_outputs.json"
    api_url = "http://localhost:8000"  # デフォルト値

    if amplify_outputs_path.exists():
        with open(amplify_outputs_path) as f:
            amplify_outputs = json.load(f)
        custom_api_url = amplify_outputs.get("custom", {}).get("ApiEndpoint", "")
        if custom_api_url:
            # 末尾のスラッシュを除去
            api_url = custom_api_url.rstrip("/")
            print(f"API URL set from amplify_outputs.json: {api_url}")
    else:
        print("Warning: amplify_outputs.json not found, using default API_URL")
